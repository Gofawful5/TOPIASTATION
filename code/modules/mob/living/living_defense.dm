...
/mob/living/bullet_act(obj/projectile/P, def_zone, piercing_hit = FALSE)
	...
	if(!P.nodamage && (. != BULLET_ACT_BLOCK))
		...
		apply_damage(P.damage, P.damage_type, def_zone, armor_check, wound_bonus=P.wound_bonus, bare_wound_bonus=P.bare_wound_bonus, sharpness = P.sharpness, attack_direction = attack_direction, white_healable = TRUE)
